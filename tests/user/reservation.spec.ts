...
test.describe('Reservation Process Test', () => {
  test('should complete the reservation process successfully', async ({ page }) => {
    ...
    await page.route('https://192.168.1.65:443/api/Reservation/check-availability', async (route) => {
      route.fulfill({
        status: 200,
    ...
    await page.route('https://192.168.1.65:443/api/Reservation/create-reservation', async (route) => {
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
